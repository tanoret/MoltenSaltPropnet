...
# ============================================================
# 3. TARGET GROUPS + TRANSFORMS
# ============================================================

TARGET_GROUPS = {
    "thermo":  ["Melt(K)", "Boil(K)"],
    "density": ["rho_a", "rho_b"],
    "mu1":     ["mu1_a", "mu1_b"],
    "mu2":     ["mu2_a", "mu2_b", "mu2_c"],
    "kcp":     ["k_a", "k_b", "cp_a", "cp_b"],
}

ALL_TARGET_COLS = [t for group in TARGET_GROUPS.values() for t in group]
...
def prepare_dataset(processor: MSTDBProcessor):
    ...
    # Build mapping from groups to column indices
    group_slices = {}
    col_index = {name: i for i, name in enumerate(target_cols)}
    for gname, gtargets in TARGET_GROUPS.items():
        idxs = [col_index[t] for t in gtargets]
        start = min(idxs)
        end = max(idxs) + 1
        group_slices[gname] = (start, end)
    ...
    # Split Y into heads for Keras multi-output
    def split_into_heads(Y_scaled):
        heads = {}
        for gname, (start, end) in group_slices.items():
            heads[gname] = Y_scaled[:, start:end]
        return heads

    Y_train_heads = split_into_heads(Y_train_scaled)
    Y_val_heads   = split_into_heads(Y_val_scaled)
    ...
    return {
        ...
        "Y_train_heads": {k: v.astype("float32") for k, v in Y_train_heads.items()},
        "Y_val_heads":   {k: v.astype("float32") for k, v in Y_val_heads.items()},
...
# ============================================================
# 5. SHARED TRUNK + MULTI-HEAD MLP
# ============================================================

def build_shared_multihead_mlp(input_dim: int, target_groups: dict) -> keras.Model:
    """
    target_groups: dict head_name -> list of target names
    """
    ...
    # Per-head small block
    for head_name, targets in target_groups.items():
        n_out = len(targets)
        h = layers.Dense(
            32,
            activation="relu",
    ...
    model = keras.Model(inputs=inputs, outputs=outputs, name="multihead_mlp")
    # One masked loss per head
    loss_dict = {head_name: masked_mse for head_name in target_groups.keys()}

# Loss weights: emphasize "good" heads, downweight noisy ones
    loss_weights = {
        "thermo":  1.0,
        "density": 1.0,
        "mu1":     1.0,
        "mu2":     0.3,   # lower influence
        "kcp":     0.5,   # medium
    }
...
def evaluate_per_target(model: keras.Model, data: dict):
    ...
    # Predictions in model space (scaled), per head
    Y_train_pred_heads_scaled = model.predict(X_train)
    Y_val_pred_heads_scaled   = model.predict(X_val)

    # Concatenate heads in the correct order to get full arrays
    def heads_to_full(pred_heads_scaled):
        parts = []
        for gname in target_groups.keys():  # preserves insertion order
            parts.append(pred_heads_scaled[gname])
        return np.concatenate(parts, axis=1)

    Y_train_pred_scaled = heads_to_full(Y_train_pred_heads_scaled)
    Y_val_pred_scaled   = heads_to_full(Y_val_pred_heads_scaled)
...
def predict_coefficients_for_all(trained: dict):
    ...
    target_groups = trained["target_groups"]
    ...
    pred_heads_scaled = model.predict(X_all)

    # concat heads → full model-space scaled
    parts = []
    for gname in target_groups.keys():
        parts.append(pred_heads_scaled[gname])
    Y_pred_scaled = np.concatenate(parts, axis=1)
...
def predict_properties_at_T(processor: MSTDBProcessor, trained: dict, T: float):
    ...
    for idx, row in df.iterrows():
        ...
        # cp_c, cp_d from data if present (else 0)
...
def train_model(processor: MSTDBProcessor, epochs: int = 400, batch_size: int = 32):
    ...
    X_train = data["X_train"]
    X_val   = data["X_val"]
    Y_train_heads = data["Y_train_heads"]
    Y_val_heads   = data["Y_val_heads"]
    target_groups = data["target_groups"]

    input_dim = X_train.shape[1]

    model = build_shared_multihead_mlp(input_dim, target_groups)
    model.summary()
